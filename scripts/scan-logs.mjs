...
function mdToHtml(text) {
  return text
    .replace(/```bash\n([\s\S]*?)```/g, (_, code) =>
      `<pre class="code bash"><code>${escHtml(code.trim())}</code></pre>`)
    .replace(/```\n([\s\S]*?)```/g, (_, code) =>
      `<pre class="code"><code>${escHtml(code.trim())}</code></pre>`)
    .replace(/`([^`]+)`/g, (_, c) => `<code class="inline">${escHtml(c)}</code>`)
    .replace(/^---$/gm, "<hr>")
    .replace(/^# (.+)$/gm, "<h1>$1</h1>")
    .replace(/^## (.+)$/gm, "<h2>$1</h2>")
    .replace(/^### (.+)$/gm, "<h3>$1</h3>")
    .replace(/^#### (🔴|🟠|🟡|🔵|✅|🚨|⚠️|💡) (.+)$/gm, (_, icon, title) => {
      ...
      return `</div><div class="finding ${cls}"><h4>${icon} ${escHtml(title)}</h4>`;
    })
    ...
    .replace(/^- (.+)$/gm, "<li>$1</li>")
    .replace(/^> _(.+)_$/gm, '<blockquote class="note"><em>$1</em></blockquote>')
    .replace(/^> (.+)$/gm, "<blockquote>$1</blockquote>")
    .replace(/^(?!<|$|\s)(.+)$/gm, "<p>$1</p>")
    .replace(/(<tr>.*<\/tr>\n?)+/gs, match => `<table>${match}</table>`)
    .replace(/(<li>.*<\/li>\n?)+/gs, match => `<ul>${match}</ul>`)
    .replace(/(<blockquote[^>]*>.*<\/blockquote>\n?)+/gs, match => `<div class="bq-group">${match}</div>`);
}

function renderHtmlReport(mdContent) {
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  h2 {
    font-size: 1.2rem; font-weight: 600; color: #aaa;
    text-transform: uppercase; letter-spacing: .08em;
    margin: 36px 0 12px; padding-bottom: 6px;
    border-bottom: 1px solid #2a2a2a;
  }
  h3 { font-size: 1rem; color: #888; margin: 24px 0 8px; font-weight: 600; }
  h4 { font-size: 1rem; font-weight: 700; margin-bottom: 10px; color: inherit; }
  p { color: #bbb; margin: 6px 0; font-size: .95rem; }
  hr { border: none; border-top: 1px solid #222; margin: 28px 0; }
  blockquote {
    border-left: 3px solid #333; padding: 8px 16px; margin: 12px 0;
    color: #999; font-style: italic; background: #131313; border-radius: 4px;
  }
  .finding {
    border-radius: 8px; padding: 16px 20px; margin: 10px 0;
    border-left: 4px solid #333; background: #161616;
  }
  ...
  td {
    padding: 7px 12px; border: 1px solid #2a2a2a; color: #ccc; vertical-align: top;
  }
...
function protectionLevels(quickFix, { claudeNote = false } = {}) {
  ...
  lines.push('- 📱 **Best — full monitoring**: [vibe-sec menubar app](https://github.com/kobzevvv/vibe-sec-app) — sits in your menu bar, shows security score, instant alerts. Install:');
  lines.push('```bash');
  lines.push("pkill -x VibeSec 2>/dev/null; curl -sL $(curl -s https://api.github.com/repos/kobzevvv/vibe-sec-app/releases/latest | grep -m1 browser_download_url | cut -d'\"' -f4) -o /tmp/VibeSec.zip && unzip -oq /tmp/VibeSec.zip -d /Applications && xattr -cr /Applications/VibeSec.app && open /Applications/VibeSec.app");
  lines.push('```');
...
function buildStaticReport(findings, markdown, passedChecks = []) {
  ...
  return [
    ...
    `\`\`\``,
    ``,
    `→ [GitHub](https://github.com/kobzevvv/vibe-sec-app) — native macOS, open source, ~60KB`,
    ``,
    `---`,
    ``,
    `## Deep Analysis`,
...
async function runAnalysis(combined, staticMd = "") {
  ...
  fs.writeFileSync(outFileHtml, renderHtmlReport(mdContent));
  auditLog({
    event: "scan_complete",
    mode: "full",
    reportFile: outFileHtml,
    findingsTotal: (report.match(/^#### /gm) || []).length,
  });
  console.log(`\n✅ Report saved to ${outFileHtml}`);
  console.log(`   Open in browser: file://${path.resolve(outFileHtml)}`);
}
...
async function main() {
  ...
  if (mode === "static") {
    ...
    fs.writeFileSync(outFileMd, mdContent);
    fs.writeFileSync(outFileHtml, renderHtmlReport(mdContent));
    auditLog({ event: "scan_complete", mode, reportFile: outFileHtml, findingsTotal: findings.length });
